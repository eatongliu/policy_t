...
public class CrossOriginFilter implements Filter {
    ...
    public void init(FilterConfig config) throws ServletException {
        ...
        if (chainPreflightConfig != null)
            LOGGER.warn("DEPRECATED CONFIGURATION: Use " + CHAIN_PREFLIGHT_PARAM + " instead of " + OLD_CHAIN_PREFLIGHT_PARAM);
        else
        ...
        if (origin != null && isEnabled(request)) {
            if (anyOriginAllowed || originMatches(allowedOrigins, origin)) {
                ...
                if (anyTimingOriginAllowed || originMatches(allowedTimingOrigins, origin)) {
                    ...
                    LOGGER.debug("Cross-origin request to " + request.getRequestURI() + " with origin " + origin + " does not match allowed timing origins " + allowedTimingOrigins);
                }
            } else {
